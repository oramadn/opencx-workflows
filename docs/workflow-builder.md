...
2. **Execution** (future Phase 3): application events fire, matching workflows are looked up, and the generated code runs inside `rivet-dev/sandbox-agent` with injected tools.
...
The generated code may **only** call functions from the injected `tools` object. These are the only bridge between the sandbox and the host.
...
4. Implement it in `mock-tools.ts` (and later in real tools for sandbox injection).
...
- Currently used by mock tools; will be used by real tools when `rivet-dev/sandbox-agent` is wired up.
...
- Designed for direct injection into `rivet-dev/sandbox-agent` in Phase 3.
...
- **`rivet-dev/sandbox-agent`:** not integrated. The mock tools + query builder are ready for injection.
...
- **Real action tools:** `sendEmail` and `sendSlackChannelMessage` are mocks. Wire up real APIs when ready.
