...
│    with Zod              │
└─────────┬────────────────┘
          │ { trigger_events, code, flow: { nodes, edges } }
          ▼
┌──────────────────────────┐
│  Postgres (workflows)    │
...
- The flow graph is stored in `workflows.flow_graph` (`JSONB`, nullable for backward compatibility).
- The LLM system prompt instructs the model to generate `flow` inside the `"ok"` response alongside `trigger_events` and `code`.
- On update, the existing flow graph is passed back to the LLM so it can maintain consistency with code changes.
- The Zod schema validates the flow graph structure on every generation response.
...
5. Parse LLM output with a Zod discriminated union on `status`:
   - `"ok"` — `trigger_events: string[]`, `code: string`, `flow: FlowGraph`. A post-parse guard also scans `code` for `tools.<method>` calls not in `VALID_TOOL_METHODS` (defense against hallucinations).
...
7. INSERT (new) or UPDATE (existing) the `workflows` row, persisting `flow_graph` as JSONB.
8. Return the full workflow record (includes `flowGraph`).
...
Three-zone layout: **React Flow canvas** (top), **chat bar** (bottom), **info panel** (right, placeholder).

```
+-------------------------------------------+--------------+
|                                            |              |
|          React Flow Canvas                 |   Info       |
...
- **Canvas:** Renders the `flowGraph` from the workflow record using `@xyflow/react`. Node positions are computed automatically by **dagre** (top-to-bottom). Three custom node types: trigger (green), condition (amber), action (blue). Non-interactive (no drag/connect) for now.
...
- **Info panel:** Right sidebar placeholder (hidden on small screens). Will host code viewer, run-test panel, and workflow metadata in a future increment.
...
- **Code viewer:** `shiki`-powered syntax highlighter. Available for the info panel in a future increment.
...
- **Info panel:** Right sidebar in the builder to display code viewer, run-test panel, and workflow metadata (placeholder already in layout).
